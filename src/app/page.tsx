...
const NICHE_PERSONAL: Record<string, string> = {
  ...
  "Manufacturing / Automation / Product Design": "For my capstone I tracked failure modes across electromechanical design iterations, with coursework in FEA, GD&T, and tolerance stack-up analysis.",
...
const NICHE_BODY_TEMPLATES: Record<string, string> = {
  ...
  "Manufacturing / Automation / Product Design": nicheTemplate(
    "For my capstone I tracked failure modes across electromechanical design iterations, with coursework in FEA, GD&T, and tolerance stack-up analysis."
  ),
  ...
  async function deleteCompany(companyname: string) {
    ...
    try {
      const res = await fetch("/api/delete-company", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ companyname, confirm: true }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full mx-4 overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="px-6 py-3 border-t border-gray-100" style={{ background: "rgba(44,15,56,0.04)" }}>
              <div className="flex items-center gap-3">
                ...
                <a href="/KOHLER_RESUME_2026.pdf" target="_blank" rel="noopener noreferrer" className="px-2.5 py-1 text-[10px] font-medium rounded-full border hover:underline flex items-center gap-1" style={{ color: "#2c0f38", borderColor: "rgba(44,15,56,0.25)", background: "rgba(44,15,56,0.06)" }}>
                  📄 Resume PDF
                </a>
