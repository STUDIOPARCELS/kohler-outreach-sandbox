...
5. What is the next best action?
   - Status: partial.
   - Evidence: `scoreJobForKohler` returns `recommended_action`; Open Roles company cards and job rows show the action.
   - Missing: action queue tables and buttons wired to draft/contact/application workflows.
...
- Gmail reply backfill and Gmail draft creation are not yet implemented.
