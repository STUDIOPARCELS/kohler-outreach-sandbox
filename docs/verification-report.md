...
- Replies/analytics UI smoke: passed in the local browser. `/replies` shows 33 outbound records, 5 threads/messages, 4 bounces, 1 out-of-office, the Add Letter Reply form, and the suppressed bounced-email panel. The bounce panel now shows replacement contact candidates for Xcimer Energy, Watson Mills, and YG Acoustics from existing `contacts` rows, while Tharp Custom Cabinetry currently has no stored alternate email. The `Use for draft` buttons are visible but were not clicked during smoke testing because clicking intentionally writes a draft contact change. `/analytics` shows 33 outbound, 8 email, 25 letters, 0 human replies, 0 letter replies, 4 bounces, 0% response rate, and 12.1% bounce rate.
