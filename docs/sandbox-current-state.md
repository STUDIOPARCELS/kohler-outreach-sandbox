...
- Reply/dashboard UI: `/replies` and `/analytics` exist and read from `sent_messages`, `email_threads`, and `email_messages`; they currently show 33 outbound records, 5 stored email events, 4 bounces, 1 out-of-office, 0 human replies, and a 12.1% bounce rate. `/replies` also includes a manual letter-reply form for responses Kohler receives outside a readable mailbox and a suppressed bounced-email panel with same-company replacement contact candidates where existing contact rows have alternate emails. Replacement candidates include a human-click `Use for draft` action that updates/creates a draft contact only; it does not send email.
